...
import { updateBlog } from "@/lib/admin/actions";
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-white tracking-tight">Edit Post</h1>
          <p className="text-sm text-white/40 mt-0.5 truncate max-w-[300px]">{blog.title_en}</p>
        </div>
        {blog.published && (
          <Link
            href={`/blog/${blog.slug}`}
      ...
      <div className="bg-[#111118] border border-white/8 rounded-2xl p-7">
        <BlogForm blog={blog} action={updateAction} isEdit />
      </div>
    </div>
  );
}
